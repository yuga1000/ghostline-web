...
app.post('/api/admin/sync-portfolio', (req, res) => {
  ...
  const pdfPath = path.join(__dirname, 'YUGA_cv_portfolio.pdf');
  if (!fs.existsSync(pdfPath)) {
    return res.status(404).json({ error: 'YUGA_cv_portfolio.pdf not found on server' });
  }
  const pdfBuffer = fs.readFileSync(pdfPath);
  const uploadUrl = SUPA_URL + '/storage/v1/object/images/portfolio/portfolio.pdf';
  ...
  const uploadReq = https.request(reqOpts, (uploadRes) => {
    let body = '';
    uploadRes.on('data', c => body += c);
    uploadRes.on('end', () => {
      if (uploadRes.statusCode >= 200 && uploadRes.statusCode < 300) {
        console.log('[sync-portfolio] Upload success:', body);
        res.json({ success: true, status: uploadRes.statusCode, response: body });
      } else {
        console.error('[sync-portfolio] Upload failed:', uploadRes.statusCode, body);
        res.status(uploadRes.statusCode).json({ error: 'Upload failed', status: uploadRes.statusCode, response: body });
      }
    });
  });
  uploadReq.on('error', (e) => {
    console.error('[sync-portfolio] Request error:', e.message);
